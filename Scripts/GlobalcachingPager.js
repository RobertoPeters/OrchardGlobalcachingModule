function setupGlobalcachingPage(CurrentPage, PageCount, TotalCount, pagerElementName, pageRequestFunction) {
    ...
    if (CurrentPage > maxPages) {
        $(pagerElementName).append('<li><a href="#" onclick="' + pageRequestFunction + '(1);">&lt;&lt;</a></li>');
        $(pagerElementName).append('<li><a href="#" onclick="' + pageRequestFunction + '(' + (startPage - 1) + ');">&lt;</a></li>');
    }
    if (PageCount > 1) {
        for (var i = startPage; i < (startPage + maxPages) && i <= PageCount; i++) {
            if (i == CurrentPage) {
                $(pagerElementName).append('<li class="active"><a href="#" onclick="' + pageRequestFunction + '(' + i + ');">' + i + '</a></li>');
            }
            else {
                $(pagerElementName).append('<li><a href="#" onclick="' + pageRequestFunction + '(' + i + ');">' + i + '</a></li>');
            }
        }

        if ((startPage + maxPages) < PageCount) {
            $(pagerElementName).append('<li><a href="#" onclick="' + pageRequestFunction + '(' + (startPage + maxPages) + ');">&gt;</a></li>');
            $(pagerElementName).append('<li><a href="#" onclick="' + pageRequestFunction + '(' + (PageCount) + ');">&gt;&gt;</a></li>');
        }
    }
}
