...
namespace Globalcaching {
    ...
    public class GlobalcachingAdminProvider : IMenuProvider
    {
        ...
        {
            builder.Add(T("Donateurs Admin"), "1.0", item => item.Action("Index", "GlobalcachingDonation", new { area = "Globalcaching" }).Permission(Permissions.GlobalAdmin));
            builder.Add(T("Shop Admin"), "1.0", item => item.Action("ShopInfo", "Shop", new { area = "Globalcaching" }).Permission(Permissions.GlobalAdmin));
